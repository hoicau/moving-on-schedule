...
const ALIASES = [
  ...
  ['结束节次', '结束节', '結束節次', '結束節', 'end', 'endperiod'],
...
export function parseRows(
  ...
  if (first < 0) throw new Error(t('文件中没有课程数据'));
  ...
  rows.slice(first + 1).forEach((row, i) => {
    ...
    try {
      ...
      validateCourse(course, totalWeeks, locale);
      ...
      errors.push(
        t('第 {0} 行：{1}', {
          0: first + i + 2,
          1: error instanceof Error ? error.message : t('数据无效'),
        }),
      );
    }
  });
  if (!count) throw new Error(t('文件只有表头，请至少填写一门课程'));
  if (count > 2000) throw new Error(t('单次最多导入 2000 条课程，请拆分文件'));
...
export async function readImport(
  ...
  if (file.size > 10 * 1024 * 1024)
    throw new Error(t('文件过大，请上传 10 MB 以内的文件'));
  if (/\.csv$/i.test(file.name)) {
    const parsed = Papa.parse<string[]>(await file.text(), {
      skipEmptyLines: true,
    });
    if (parsed.errors.length)
      throw new Error(t('CSV 格式错误：{0}', { 0: parsed.errors[0].message }));
    return parseRows(parsed.data, totalWeeks, 'CSV', locale);
  }
  if (!/\.xlsx$/i.test(file.name))
    throw new Error(
      t('支持 .xlsx 和 UTF-8 .csv 文件；旧版 .xls 请先用 Excel 另存为 .xlsx'),
    );
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load(await file.arrayBuffer());
  const sheet = workbook.worksheets.find((s) => s.actualRowCount > 0);
  if (!sheet) throw new Error(t('工作簿中没有数据'));
  if (sheet.rowCount > 2001)
    throw new Error(t('单次最多导入 2000 条课程，请拆分文件'));
  const rows: unknown[][] = [];
  sheet.eachRow({ includeEmpty: true }, (row) => {
    rows.push(Array.isArray(row.values) ? row.values.slice(1) : []);
  });
  return parseRows(rows, totalWeeks, sheet.name, locale);
}
...
export function headersFor(locale: Locale): string[] {
  if (locale === 'en')
    return ['name', 'day', 'start', 'end', 'weeks', 'room', 'teacher', 'note'];
  const t = createTranslator(locale);
