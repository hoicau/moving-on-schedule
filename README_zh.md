...
- 导出与恢复完整 JSON 备份，包含设置和偏好，并验证 SHA-256 完整性。
- 本地保存回读校验、旧数据迁移、原始数据下载及多标签页覆盖保护。
...
- [使用](docs/usage_zh.md)
- [JSON 备份与存储安全](docs/backup-format_zh.md)
- [部署指南](docs/deployment_zh.md)
- [开发与验证](docs/development_zh.md)

课程数据保存在当前浏览器，暂无账号或云同步。清除站点数据或更换域名前，请先导出 JSON 备份。无痕模式可能在关闭窗口后删除本地数据。表格导入需遵循文档中的逐行格式。
